...
Usage:
    python -m src.metrics.setup_and_run               # run all scenarios
    python -m src.metrics.setup_and_run --explore     # list movies by genre
"""
...
def load_data():
    ...
    if not path.exists():
        print(f"[ERROR] {path} not found — run: python -m src.data.preprocess")
        return None
    df = pd.read_csv(path)
...
def load_models():
    ...
    if not vec_path.exists() or not mat_path.exists():
        print("[ERROR] Model files missing — run: python -m src.data.preprocess")
        return None, None
...
def main():
    ...
    parser.add_argument(
        "--explore", action="store_true", help="Explore dataset instead of evaluating"
    )
