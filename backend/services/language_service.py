...
_STOPWORDS = {
    "es": {"quiero", "una", "un", "de", "que", "me", "para", "con", "por", "los", "las",
           "como", "algo", "es", "en", "del", "al", "más", "muy", "película", "pelicula",
           "recomienda", "recomiéndame", "recomiendame", "ver", "busco", "dame"},
    "fr": {"je", "veux", "un", "une", "de", "du", "des", "les", "la", "le", "avec",
           "pour", "que", "est", "dans", "film", "quelque", "chose", "comme", "voire"},
    "pt": {"quero", "um", "uma", "de", "que", "para", "com", "por", "os", "as",
           "como", "algo", "em", "do", "da", "ao", "mais", "muito", "filme"},
}
...
_GENRE_SYNONYMS: dict[str, list[str]] = {
    ...
    "action":    ["action", "acción", "accion", "action", "ação", "thrilling",
                  "explosion", "fight"],
    "drama":     ["drama", "emotional", "dramática", "dramatique", "sad", "touching"],
    "horror":    ["horror", "terror", "scary", "spooky", "horreur", "miedo",
                  "creepy", "aterradora"],
    "sci-fi":    ["sci-fi", "science fiction", "ciencia ficción", "ciencia ficcion",
                  "science-fiction", "space", "alien", "future", "futurista", "robot"],
    "romance":   ["romance", "romántica", "romantica", "romantique", "love", "amor",
                  "couple", "amorosa", "heart"],
    "thriller":  ["thriller", "suspenso", "suspense", "mystery", "misterio", "crime",
                  "crimen", "intriga"],
    "animation": ["animation", "animación", "animacion", "animé", "anime",
                  "animated", "cartoon", "dibujos"],
    "fantasy":   ["fantasy", "fantasía", "fantasia", "fantaisie", "magic", "magia",
                  "wizard", "dragon", "mágica"],
    "family":    ["family", "familia", "familial", "niños", "children", "kids",
                  "infantil", "familiar"],
...
_MOOD_SYNONYMS: dict[str, list[str]] = {
    "dark":      ["dark", "oscura", "sombre", "gloomy", "intense", "gritty",
                  "oscuro", "pesada"],
    "feel-good": ["feel-good", "happy", "feliz", "inspiring", "uplifting",
                  "cheerful", "alegre", "positiva", "divertida", "feel good"],
    "intense":   ["intense", "fast-paced", "intensa", "breathless", "edge of my seat",
                  "emocionante", "acelerada"],
    "relaxing":  ["relaxing", "chill", "calm", "light-hearted", "relajante",
                  "tranquila", "ligera", "suave"],
    "nostalgic": ["nostalgic", "nostálgica", "nostalgique", "classic feel",
                  "clásica", "clásico", "old days"],
    "romantic":  ["romantic", "romántica", "romantique", "love story",
                  "historia de amor", "amorosa"],
    "scary":     ["scary", "aterradora", "de miedo", "que da miedo", "terror",
                  "espeluznante"],
    "funny":     ["funny", "chistosa", "cómica", "comica", "graciosa", "de risa",
                  "jocosa", "comique"],
}
...
_DECADE_MAP: dict[str, tuple[int, int]] = {
    ...
    "los 2000": (2000, 2009), "los 2010": (2010, 2019), "los 2020": (2020, 2029),
    "los 90": (1990, 1999), "los 80": (1980, 1989),
    "los 70": (1970, 1979), "los 60": (1960, 1969), "los 50": (1950, 1959),
    "años 50": (1950, 1959), "años 60": (1960, 1969), "años 70": (1970, 1979),
    "años 80": (1980, 1989), "años 90": (1990, 1999), "años 2000": (2000, 2009),
}

_LANGUAGE_SYNONYMS: dict[str, str] = {
    "spanish": "es", "español": "es", "espanol": "es", "en español": "es",
    "english": "en", "inglés": "en", "ingles": "en", "en inglés": "en",
    "french": "fr", "francés": "fr", "frances": "fr", "en francés": "fr",
    "italian": "it", "italiano": "it", "en italiano": "it",
...
def normalize(raw_text: str) -> dict:
    ...
    for genre, synonyms in _GENRE_SYNONYMS.items():
        for syn in synonyms:
            if _normalize_text(syn) in normalized and genre not in mapped_genres:
                mapped_genres.append(genre)
                break

    mapped_moods: list[str] = []
    for mood, synonyms in _MOOD_SYNONYMS.items():
        for syn in synonyms:
            if _normalize_text(syn) in normalized and mood not in mapped_moods:
                mapped_moods.append(mood)
                break

    mapped_year_range: list[int] | None = None
    for decade_key, year_range in _DECADE_MAP.items():
        if _normalize_text(decade_key) in normalized:
            mapped_year_range = list(year_range)
            break
    if mapped_year_range is None:
        # Try explicit year "2000" → [2000, 2009] if no range found
        year_match = re.search(r"\b(19\d{2}|20\d{2})\b", raw_text)
        if year_match:
            y = int(year_match.group(1))
            mapped_year_range = [y, y]

    mapped_language: str | None = None
    for phrase, code in _LANGUAGE_SYNONYMS.items():
        if _normalize_text(phrase) in normalized:
            mapped_language = code
            break
